// AUTO-GENERATED EDGE FUNCTION — IP-PROTECTED SIMULATION ENGINE
// Bundles simulation + market analysis logic server-side.
// Do not edit individual sections; regenerate from src/lib/* if logic changes.
...
serve(async (req) => {
  ...
  try {
    ...
    const { action, gameConcept } = body ?? {};

    if (!gameConcept || typeof gameConcept !== "object" || !gameConcept.gameName) {
      return new Response(
        JSON.stringify({ error: "Invalid game concept" }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    if (action === "marketAnalysis") {
      const result = await runMarketAnalysis(gameConcept);
    ...
    const result = runSimulation(gameConcept);
    return new Response(JSON.stringify(result), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
